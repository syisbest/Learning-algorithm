# Learning-algorithm

## 3n+1 偶数除二，奇数*3+1，最后结果为1的一个序列

## 8queen 八皇后问题

## add_ans 加法进位的次数计算

## add_to_mul 1-49累加中变两个加号为乘号，结果变为2015

## ant_cold 蚂蚁感冒，一根绳若干蚂蚁，相遇就掉头，有一只感冒和其它相遇就传染，全走出绳后有多少感冒

## array_move 移动数组中的数

## array_sum 序列求和，采用树型数组

## bfs_grass 草地扩散，每次往上下左右长草，求k天后的结果

## big_fbnq_div 很大的斐波那契数且最后两位相除

## bign 实现了数组存储大数的结构体

## bsearch 二分查找

## bsearch_LR 二分查找，当查找的数出现多次时，返回区间

## cantor 串线数表查找第n个数

## climb_stairs 爬楼题问题，每次爬一阶或两阶到39楼且步数为偶数，问能有多少中方案

## connected_block 在图里连接一部分边，连接到一起的为一个块，求一共有几个块

## continuity_interval 判断一个序列是否能经过变换变为连续序列

## cut_chocolate 剪巧克力，将n块长方形巧克力分成k块一样大的正方形，求最大的分法

## cut_stamp 剪邮票，从3*4一张的邮票中剪出五张连通的，求剪法数

## date 日期问题，输入一个不确定格式的日期如02/03/04，输出其可能的结果

## dfs_and_cache dfs中采用记忆数组来减枝，拿宝物，一共能拿k个，每次只能向右或向下，下一个宝物价值大于之前所有，可以选则拿或不拿

## dfs_mutiple 多次调用dfs，找到不同的起点调用dfs，将字符g周围都变成g

## dfs_tree 对树进行dfs，将权重大于0的叶子加到根上去，输出最后的树里最大的权重

## equation 凑算式，使其和等于10的方案个数

## especial_fractional 特别的分数，a/b*c/d=(ac)/(bd)

## f 蓝桥F题 1到n中含2 0 1 9这四个数字的和

## factorial 阶乘计算结果用数组保证

## factorial_prime n的阶乘可以写成多个素数的积，求出每个素数的个数

## fast_power_ex 快速幂求(x+m*10^k)%n

## find_wrong 在连续序列中寻找错误的数，有一个重复和断点

## four_quadratic_sum 四平方和，求一个数的四平方和形式

## g 蓝桥G题 求一个完全二叉树结点和最大的那一层

## h 蓝桥H题 给部分等差数列的数，求补全后数的个数

## hard_string 由n个字母搭配形成l长的字符串，串中没有子串循环

## island 海岛周围有水就会被淹没（#周围有.），求被淹没的岛的个数

## lcs 最大公共子串和最大公共子序列

## libai_dajiu 店5花10，遇到店酒*2，遇到花酒-1，酒初始2，最后一次是花，酒恰好为0。求满足的次数

## list_ex 链表使用案例：移动数据，删除后插入

## lower_upper 使用内置函数计算数组一个区域中数的个数

## make_up_num 由n个数组成其他的数，求组成不了的数的个数

## map_ex 图使用案例：找八连通区域

## max_ratio 求一个不完整等比数列的最大公比

## maxsum 用分治法求数组的最大连续和

## merge_sort 归并排序

## merge_sort_count 利用归并排序找到满足(i<j,A[i]>A[j])的个数

## mineweep 扫雷，周围一圈有几个*就让该位置数字为几

## mixed_number 将一个数写成带分数形式，又多少种写法

## n_sum 数组中n个数的和为sum

## palindrome_number 输出一个回文数且各个位的和等于一个给定的数

## people_line 乱序变增序，人每交换一次，不满意度递增(+1，+2，+3...)，只能挨着的交换，求最后人们的不满意度和

## point_n 两数相除，求小数点后第n位到n+3位

## prime_loop 一个由数字1到n形成的环，环内相邻两位数之和为素数

## print_permutation 输出从1到n的全排列

## print_permutation_new 从一个数组中取数全排列

## print_subset 输出0到n集合的子集 增量和位向量法

## print_subset_new 输出0到n集合的子集 二进制法

## q6174 一个数最大排列减最小排列等于本身

## qsort_char 在一串英文单词中匹配一个字母组成相同的单词

## queue_ex 队列使用案列：队首输出，下一张放到队尾

## quick_select 利用快速排序找第k小的数

## quick_sort 快速排序

## reverse_c 翻转纸牌，每次翻转两个相邻纸牌，翻转成指定样式需要多少次

## reverse_num 交换数字，使乱序1-n序列交换后有序最少要交换多少次

## six_angle_sum 六角和，固定三个数，求其他数使每一行和相等

## snake_array 蛇形数组，给出宽度和数字求坐标

## snake_coordinate 蛇形坐标，在二维坐标系的蛇形图，求点到原点的长度

## stack_ex 栈使用案例 火车进出站车厢编号

## statistics_like 统计点赞数，求在一个时间区间点赞数超过一定数量的帖子

## strequl 一个串的最小周期

## sum_k 连续序列之和是k的倍数，求满足的序列有多少

## take_cards 拿扑克牌，拿13张牌一共有多少中拿法

## three_array_increase 三个数组，分别取一个数出来时递增的，有多少种取法

## trangle_S 一个三角形圈住了多少个点

## tree_ball 一个二叉树，小球从根向下滚，滚到的结点改变一次状态（初始全关），滚到关的就往左，开的往右

## tree_ex  二叉树的生成，添加，删除和遍历结点

